    ...
    <div class="fixed inset-0 z-50 flex items-end justify-center bg-ledger/50">
      <section class="sheet-enter w-full max-w-md rounded-t-[1.5rem] bg-white p-6">
        ...
        <div class="mt-3 rounded-xl bg-mist/60 px-4 py-3">
          <p class="truncate text-sm font-bold text-ledger">${escapeHtml(label)}</p>
  ...
  if (result.intent === 'pickup') {
    const rows = result.todayOrders.length ? result.todayOrders.map((order) => `
        <div class="mt-2 flex items-center justify-between rounded-xl bg-mist/60 px-3 py-2.5">
          <div class="min-w-0"><p class="text-sm font-bold text-ledger">${escapeHtml(order.storeName)}</p><p class="truncate text-xs text-slate-500">${escapeHtml(order.itemName)}${order.selectedOptions.length ? '（' + escapeHtml(order.selectedOptions.map((o) => o.name).join('、')) + '）' : ''} · $${money(order.totalPrice)}</p>${order.note ? `<p class="mt-0.5 text-xs font-bold text-stamp">備註：${escapeHtml(order.note)}</p>` : ''}</div>
  ...
  if (result.intent === 'pay') {
    const rows = result.unpaidOrders.length ? result.unpaidOrders.map((order) => `
        <div class="mt-2 flex items-center justify-between rounded-xl bg-amber-50 px-3 py-2.5">
          <div class="min-w-0"><p class="text-sm font-bold text-ledger">${escapeHtml(order.orderDate)} ${escapeHtml(order.storeName)}</p><p class="truncate text-xs text-slate-500">${escapeHtml(order.itemName)}${order.selectedOptions.length ? '（' + escapeHtml(order.selectedOptions.map((o) => o.name).join('、')) + '）' : ''}</p>${order.note ? `<p class="mt-0.5 text-xs font-bold text-stamp">備註：${escapeHtml(order.note)}</p>` : ''}</div>
          <div class="ml-2 flex shrink-0 items-center gap-2">
    ...
    ${result.todayOrders.length ? `
      ...
      ${result.todayOrders.map((order) => `
        <div class="mb-2 flex items-center justify-between rounded-xl bg-mist/60 px-3 py-2.5">
          <div><p class="text-sm font-bold text-ledger">${escapeHtml(order.storeName)}</p><p class="text-xs text-slate-500">${escapeHtml(order.itemName)}${order.selectedOptions.length ? '（' + escapeHtml(order.selectedOptions.map((o) => o.name).join('、')) + '）' : ''} · $${money(order.totalPrice)}</p>${order.note ? `<p class="mt-0.5 text-xs font-bold text-stamp">備註：${escapeHtml(order.note)}</p>` : ''}</div>
