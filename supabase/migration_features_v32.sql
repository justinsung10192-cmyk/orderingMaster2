-- ============================================================================
--  v3.2.0 功能遷移：素食標示、請假、推薦菜單、自訂欠費(使用者彼此)、更新日誌、
--                部分繳費、請客場次（管理員設定金額上限）
...
-- 2) 自訂欠費（使用者彼此新增，由新增者核銷）---------------------------------
drop table if exists public.custom_debts cascade;
create table if not exists public.custom_debts (
  ...
  creditor_id bigint not null references public.users(id) on delete cascade,  -- 新增者（可核銷）
  debtor_id   bigint not null references public.users(id) on delete cascade,  -- 欠費者
  amount      numeric(10,2) not null,
  note        text not null default '',
  status      text not null default 'Outstanding',   -- Outstanding / Settled
...
-- 3) 請客場次：直接加在 sessions（不再使用獨立的 treats 表）-------------------
...
-- 4) 訂單：移除舊 treat_id，保留 treat_covered（本單免費金額）--------------------
alter table public.orders drop column if exists treat_id;
alter table public.orders add column if not exists treat_covered numeric(10,2) not null default 0;

-- 5) 移除舊的 treats 表（已由場次取代）----------------------------------------
...
-- 10) 重建 fn_settle_order：支援請客場次（免費額度由場次上限與剩餘決定）--------
...
values
('', 'v3.2.0', '請假／推薦菜單／自訂欠費／請客場次',
 '新增：請客場次（管理員設定金額上限、超過補差價、免費統計）、請假取消（9:00 前申請、批准後退費）、使用者推薦菜單、自訂欠費（使用者彼此新增、新增者核銷）、部分繳費、素食標示、AI 圖片辨識設定、可拉動捲軸、師長帳號、載入動畫。'),
('', 'v3.1.0', '行事曆與通知優化',
