...
create table if not exists public.menu_items (
  ...
  dish       text not null default '',      -- 當天菜色（每日菜單用，僅顯示）
...
alter table public.menu_items add column if not exists dish text not null default '';
...
-- ============================================================================
--  v3.2.0 新增（素食、請客、自訂欠費、請假、推薦、更新日誌、部分繳費）
-- ============================================================================
...
create or replace function public.fn_partial_pay(
  p_class_id text, p_user_id bigint, p_order_id bigint, p_amount numeric
) returns jsonb
language plpgsql security definer set search_path = public
as $$
declare
...
begin
  select * into v_order from orders
  where id = p_order_id and user_id = p_user_id and class_id = p_class_id for update;
  if v_order.id is null then raise exception 'ORDER_NOT_FOUND'; end if;
  if v_order.is_deleted then raise exception 'ORDER_NOT_FOUND'; end if;
  ...
  if v_new_paid >= v_order.total_price then
    update orders set prior_paid = total_price, payment_status = 'PaidCash', updated_at = now() where id = p_order_id;
  else
    update orders set prior_paid = v_new_paid, payment_status = 'PartiallyPaid', updated_at = now() where id = p_order_id;
  end if;
  insert into transactions (class_id, user_id, order_id, amount, kind, note)
  values (p_class_id, p_user_id, p_order_id, -v_apply, 'Cash', '部分繳費');
  return jsonb_build_object('ok', true, 'applied', v_apply, 'outstanding', round((v_order.total_price - v_new_paid)::numeric, 2), 'payment_status', (v_new_paid >= v_order.total_price)::boolean);
end;
$$;
...
begin
  ...
  if p_pure_mode then
    if p_cash_outstanding > 0 then raise exception 'PURE_MODE_NO_CASH'; end if;
    if v_balance + v_wallet_paid < p_wallet_paid then raise exception 'INSUFFICIENT_BALANCE'; end if;
  end if;

  v_balance := v_balance + v_wallet_paid;
  if p_wallet_paid > 0 then
    if v_balance < p_wallet_paid then raise exception 'INSUFFICIENT_BALANCE'; end if;
    v_balance := v_balance - p_wallet_paid;
  end if;
  ...
  if v_wallet_paid > 0 then
    insert into transactions (class_id, user_id, order_id, amount, kind, note) values (p_class_id, p_user_id, v_order_id, v_wallet_paid, 'Refund', '訂單修改退款');
  end if;
  if p_wallet_paid > 0 then
...
-- ============================================================================
-- 退款修正（與 migration_refund_full.sql 一致）：退還「實際支付」＝儲值金＋現金−請客免費額度
-- ============================================================================
create or replace function public.fn_refund_order(
  p_class_id text,
  p_user_id bigint,
...
-- ============================================================================
-- 錢包系統修正：結清欠費時優先以餘額扣抵（與 migration_wallet_auto_offset.sql 一致）
-- ============================================================================
create or replace function public.fn_settle_cash(
  p_class_id text,
  p_user_id bigint,
